...
class StructInterfaceSource(IObjectSource):
    impl: testbed1.api.IStructInterface
    def __init__(self, impl: testbed1.api.IStructInterface):
        self.impl = impl
    ...
    def olink_collect_properties(self) -> object:
        ...
        props["propBool"] = testbed1.api.from_struct_bool(v)
        v = self.impl.get_prop_int()
        props["propInt"] = testbed1.api.from_struct_int(v)
        v = self.impl.get_prop_float()
        props["propFloat"] = testbed1.api.from_struct_float(v)
        v = self.impl.get_prop_string()
        props["propString"] = testbed1.api.from_struct_string(v)
        return props
...
class StructArrayInterfaceSource(IObjectSource):
    impl: testbed1.api.IStructArrayInterface
    def __init__(self, impl: testbed1.api.IStructArrayInterface):
        self.impl = impl
        impl.on_prop_bool_changed += self.notify_prop_bool_changed
        impl.on_prop_int_changed += self.notify_prop_int_changed
        impl.on_prop_float_changed += self.notify_prop_float_changed
    ...
    def olink_collect_properties(self) -> object:
        props = {}
        v = self.impl.get_prop_bool()
        props["propBool"] = [testbed1.api.from_struct_bool(_) for _ in v]
        v = self.impl.get_prop_int()
        props["propInt"] = [testbed1.api.from_struct_int(_) for _ in v]
        v = self.impl.get_prop_float()
